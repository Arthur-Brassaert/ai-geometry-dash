...
def main():
    parser = argparse.ArgumentParser(description='Train Geometry Dash AI with GPU')
    parser.add_argument('--timesteps', type=int, default=100000, help='Total training timesteps')
    parser.add_argument('--n_envs', type=int, default=4, help='Number of parallel environments')
    parser.add_argument('--learning_rate', type=float, default=0.0003, help='Learning rate')
    parser.add_argument('--gamma', type=float, default=0.99, help='Discount factor')
    parser.add_argument('--model_name', type=str, default='geometry_dash_ai_gpu', help='Name for saved model')
    ...
    if torch.cuda.is_available():
        device = 'cuda'
        # Optional: set the GPU device index if you have multiple GPUs
    ...
    # Using a smaller network to save GPU memory
    model = PPO(
        "MlpPolicy",
        env,
        ...
        policy_kwargs=dict(
            net_arch=[128, 128]  # Smaller network to fit GPU memory
        ),
